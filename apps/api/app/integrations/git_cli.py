...
class GitCliBackend:
    ...
    def archive_refs(
        ...
        """Return two git-archive TAR payloads for ProjectStorage publication."""
        ...
        if local.exists():
            return archive_from(local, base_ref), archive_from(local, revised_ref)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            if result.returncode != 0:
                raise AppError(
                    "GIT_ERROR",
                    f"clone failed: {result.stderr or result.stdout}",
                    status_code=400,
                )
            subprocess.run(
            ...
            return archive_from(clone, base_ref), archive_from(clone, revised_ref)
    ...
    def _repo_path(self, project_id: str) -> Path:
        ...
        return project.local_path(project.layout.project)
    ...
    def ensure_repo(self, project_id: str) -> Path:
        ...
        repo = self._repo_path(project_id)
        git_dir = repo / ".git"
    ...
    def _resolve_repo(self, project_id: str) -> tuple[Path, str | None, str]:
        ...
        meta = self._project(project_id).load_project_meta()
    ...
    def commit(
        ...
        if mode == "local":
            ...
            if sync_from_merged:
                targets = paths
                if not targets:
                    targets = project.list_files(TreeRef.work())
                for rel in targets:
            ...
            if sync_from_merged:
                ...
                for rel in targets:
                    ...
                    dest.write_bytes(project.read_bytes(TreeRef.work(), rel))
    ...
    def restore(
        ...
        if repo_mode == "local":
            if paths:
                ...
                if proc.returncode == 0:
                    restored = project.list_files(TreeRef.work())
        else:
            if paths:
                for rel in paths:
                    rel = rel.replace("\\", "/").lstrip("/")
                    ...
                    if raw.returncode == 0:
                        project.write_bytes(TreeRef.work(), rel, raw.stdout)
                        restored.append(rel)
            else:
                ...
                restored = ArchiveTransfer.import_tar(
                    project,
                    TreeRef.work(),
                    proc.stdout,
                    label=f"git restore {ref}",
                    subdir=subdir,
    ...
    def show_slice(
        ...
        content, _encoding = decode_text_bytes(raw)
    ...
    def zone_from_commit(
        ...
        zs = ZoneService(self.settings, self.storage)
        ...
        zid = zmeta["id"]
        files = ArchiveTransfer.import_tar(
            project,
            TreeRef.zone(zid),
            archive_bytes,
            label=f"git zone {ref}",
            subdir=strip_prefix if mode == "external" and strip_prefix else None,
            strip_prefix=strip_prefix if mode == "local" else None,
            exclude_top_levels={
                "base",
                "revised",
                "zones",
            ...
            exclude_paths={".gitignore", "meta.json"} if mode == "local" else None,
            hoist_single_root=True,
            max_expanded_bytes=self.settings.max_upload_mb * 1024 * 1024,
        )
        return {
