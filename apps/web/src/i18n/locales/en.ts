export default {
  app: {
    title: "paper-diff",
  },
  toolbar: {
    importProject: "Import project",
    importWork: "Import project",
    addZone: "Add compare zone",
    exportWork: "Export project",
    base: "base",
    revised: "revised",
    importZips: "Import dual zips",
    advancedDualZip: "Advanced: dual zip",
    gitRepoPlaceholder: "git repo path or URL",
    baseRefPlaceholder: "base ref",
    revisedRefPlaceholder: "revised ref",
    subdirPlaceholder: "subdir",
    importGit: "Import git",
    acceptFile: "Accept file",
    undo: "Undo",
    compile: "Compile",
    latexdiffPdf: "latexdiff PDF",
    exportMerged: "Export project",
    acceptReport: "Accept report",
    autoCompile: "auto-compile",
    rootSelect: "TeX root",
    rootPlaceholder: "Select compile entry…",
    rootRecommended: "recommended",
    toggleFiles: "Files",
    togglePdf: "PDF",
    toggleBottom: "Log",
    resetLayout: "Reset layout",
    gitCommit: "Git commit",
    gitStatus: "Git status",
    layoutPreset: "Layout preset",
    presetDefault: "Default three panes",
    presetEditorPdf: "Editor + PDF",
    presetFilesEditor: "Files + editor",
    commandPalette: "Command palette",
  },
  filter: {
    sentences: "sentences",
    words: "words",
    hunks: "hunks",
    all: "all units",
  },
  panels: {
    files: "Files",
    compileErrors: "Compile errors",
    comparer: "Comparer",
    comparerEmpty: "Open a file to compare",
    swapSides: "Swap sides",
    diffHeader: "Comparer",
    diffHeaderWith: "Comparer · {left} ↔ {right}",
    diffHeaderPreview: "Comparer · commit {base} ↔ {revised}",
    pdfPreview: "PDF preview",
    pdfOfFile: "PDF · {path}",
    pdfCompile: "PDF · compile output",
    compileLog: "Compile log…",
    explorer: "Explorer",
    zones: "Zones",
    git: "Git",
    compile: "Compile",
    agent: "Agent",
    settings: "Settings",
    bottomLog: "Output",
    bottomLocked: "Pinned · Ctrl+`",
    sideProject: "from current project",
    sideZone: "from compare zone",
    sideGit: "from git commit",
    dragToRearrange: "Drag title bar to rearrange panes",
    dragBottomResize: "Drag to resize bottom panel height",
  },
  preview: {
    binaryImage: "Binary image — text diff unavailable",
    binaryNotText: "Binary / non-text file — cannot show in editor",
    workImage: "Project (work)",
    zoneImage: "Compare zone",
    loading: "Loading…",
    imageLoadError: "Failed to load image",
    docxNotFound:
      "Word file not found (404). If the name was garbled after zip import, re-import with the fixed zip decoder.",
    docxUnsupported:
      "This path cannot be raw-previewed as Word (extension or encoding issue).",
    docxEmpty: "Word file is empty or failed to download.",
    docLegacyUnsupported:
      "Legacy .doc is not previewable here — re-save as .docx in Word. .docx is rendered in-browser and may differ slightly from desktop Word.",
  },
  viewer: {
    mdPreview: "Preview",
    mdSource: "Source",
    tableView: "Table",
    tableMeta: "{cols} cols · {rows} rows",
    tableTruncated: "first {n} shown",
    tableEmpty: "Empty table",
    notebookView: "Notebook",
    notebookMeta: "{n} cells · {lang}",
    notebookEmpty: "Empty notebook",
    notebookParseError: "Failed to parse notebook",
    notebookMime: "Output mime: {keys}",
    logIssues: "Issues",
    logErrors: "{n} errors",
    logWarnings: "{n} warnings",
    logClean: "No parsed errors/warnings",
    logNoIssues: "No matching issues",
    logFilterAll: "All",
    logFilterErrors: "Errors",
    logFilterWarnings: "Warnings",
    logSevError: "Error",
    logSevWarning: "Warn",
  },
  tools: {
    strip: "Tools",
    comparer: "Comparer",
    editor: "Editor",
    pdf: "PDF preview",
    word: "Word preview",
    output: "Output",
    empty: "empty",
    close: "Close",
    dropHint: "Drop a file from the tree here, or open {tool} from the toolbar",
    unsupportedFile:
      "This {tool} cannot open “{file}”. Choose another file. The target pane was not changed.",
    dirtyAcceptConfirm:
      "This file has unsaved edits. Accept-all will overwrite them. Continue?",
    closeDirtyConfirm: "Save failed. Close the tab anyway?",
  },
  workbench: {
    dragColumn: "Drag empty tab-bar gutter to move the whole column",
    emptyColumn: "Empty column",
  },
  import: {
    addFiles: "Add files to project",
    conflictTitle: "File conflicts",
    conflictSummary:
      "{total} files: {conflicts} conflict with existing paths, {fresh} new.",
    defaultAction: "Default for conflicts",
    overwrite: "Overwrite",
    skip: "Skip",
    rename: "Rename",
    applyAll: "Apply to all conflicts",
    cancelAll: "Cancel import",
    continueImport: "Continue import",
  },
  importModal: {
    open: "Import",
    projectTitle: "Import project",
    zoneTitle: "Import compare zone",
    name: "Name",
    methodZip: "ZIP",
    methodFolder: "Folder",
    methodFiles: "Files",
    analyzing: "Analyzing…",
    preview: "Import preview",
    previewSummary:
      "{total} path(s) · ~{fresh} new · {conflicts} conflict(s)",
    morePaths: "+{n} more path(s)",
    conflictNote:
      "Conflicting paths will be overwritten on confirm by default.",
    cancel: "Cancel",
    confirm: "Confirm import",
    modeProject: "Import as project",
    modeZone: "Import as compare zone",
    modeSupplement: "Supplement project work",
    replaceHint: "This will rebuild the project work tree from the payload.",
    replaceWarning:
      "Replace the current project work tree with this import? Existing files will be overwritten.",
    zoneHint: "Creates a new compare zone; does not modify project work.",
    advanced: "Advanced…",
    advancedDual: "Dual ZIP (compat)",
    advancedGit: "Git two-ref",
    gitLocalHint: "Prefer a local repo path; remote without auth may fail.",
    noFiles: "No files selected to import",
  },
  agent: {
    title: "Agent",
    analyze: "Analyze current file",
    propose: "Propose draft",
    apply: "Apply proposal",
    applyConfirm: "Apply proposed content to the work file?",
    summary: "Summary",
    leftStrengths: "Left strengths",
    rightStrengths: "Right strengths",
    risks: "Risks",
    recommendations: "Recommendations",
    rationale: "Rationale",
    empty: "Open a compared file, then run Analyze",
    notConfigured: "Agent not configured (endpoint missing or provider off)",
    needFile: "Open a file with left/right content first",
    needProposal: "Run Propose before Apply",
    analyzeOk: "Analysis ready",
    proposeOk: "Draft proposed",
    applyOk: "Applied agent draft → rev {revision}",
    instruction: "Optional instruction",
    instructionPlaceholder: "e.g. prefer concise methods wording",
    chatSend: "Send",
    chatPlaceholder: "Ask about the open file…",
    chatOk: "Agent replied",
    providerStub: "stub",
    providerOff: "off",
    providerHttp: "http",
    chatTitle: "Chat",
  },
  csv: {
    preview: "Table diff",
    run: "Preview CSV changes",
    changed: "{n} changed rows",
    empty: "No row differences in preview window",
    row: "row {n}",
    status: "status",
  },
  zones: {
    empty: "No compare zones yet",
    rename: "Rename",
    delete: "Delete",
    fromWork: "Snapshot current project",
    fromFolder: "Add from folder",
    fromZip: "Add from zip",
    namePrompt: "Zone name",
    deleteConfirm: "Delete this compare zone?",
    fileCount: "{n} files",
    snapshotOk: "Snapshot created: {name}",
    importOk: "Imported zone {name} ({n} files)",
    treeEmpty: "No files in this zone",
    toggleTree: "Expand / collapse file tree",
  },
  fileActions: {
    add: "Add→project",
    delete: "Delete from project",
    replaceAll: "Replace all",
  },
  units: {
    accept: "Accept {granularity}",
    empty: "No units (open a compared modified file)",
    sentence: "sentence",
    word: "word",
    hunk: "hunk",
  },
  status: {
    added: "added",
    removed: "removed",
    modified: "modified",
    same: "same",
    unknown: "pending",
    skipped: "skipped",
    pending: "pending",
    queued: "queued",
    comparing: "comparing",
    ready: "ready",
  },
  errors: {
    selectZips: "Select base.zip and revised.zip",
    selectWorkZip: "Select a project zip to import",
    fillGit: "Fill repo path/url and both refs",
    selectRoot: "Select a main .tex entry before compile",
  },
  pdf: {
    empty: "No PDF — open or drop a .pdf from the tree",
    noContainer: "Preview host not ready",
    zoomReset: "Reset",
    zoomHint: "Ctrl+scroll to zoom",
    panHint: "Ctrl+drag to pan when overflow",
  },
  comparer: {
    pickTarget: "Pick compare target",
    vs: "vs",
    sourceZone: "Compare zone",
    sourceGit: "Git commit",
    pickZone: "Choose zone…",
    zonePath: "Path in zone",
    gitRef: "Commit",
    gitPath: "Path in repo",
    pickCommit: "Choose commit…",
    applyTarget: "Apply target",
    memoryHint:
      "Saved as project default and remembered for this file. Opening other files uses the default source with the current path.",
    rememberedFile: "Remembered for this file",
    rememberedProject: "Project default",
    needActiveZone: "Pick a zone file from a zone tree into the compare side.",
    needZoneId:
      "Choose a file from a specific compare-zone tree (zones are isolated snapshots).",
    needBothSides:
      "Set both compare sides: project file on the left; use “vs…” or drop a zone/Git file for the right. Diff and accept appear when both are ready.",
    dropWork: "Drop a project file, or use “Add to compare” in the file tree",
    dropCompare: "Drop a zone file, or pick a zone/Git target via “vs…”",
    oneSideOnly:
      "Only one side is set: view/edit the bound side; drop or “Add to compare” fills the empty side.",
    replacedSide: "Replaced comparer {side} side: {path}",
    sideWork: "project",
    sideCompare: "compare",
    noFocusedComparer: "No comparer focused — opened a new one.",
    fromProject: "from current project",
    fromZone: "from compare zone",
    fromGit: "from git {ref}",
    pickProject: "Pick from current project…",
    pickFromZone: "Pick from compare zone…",
    pickFromGit: "Pick from git commit…",
    emptyProjectHint: "No project file bound yet. Open the explorer and choose one.",
    emptyCompareHint:
      "No compare target yet. Pick a file from a zone or a git commit.",
    loadFailedShort: "load failed",
    workLoadFailed:
      "Could not load the work file (project not ready, missing path, or not text). Pick the file again.",
    compareLoadFailed:
      "Could not load the compare file (missing zone/git path or binary). Pick a compare target again.",
    compareBinary: "That compare file is binary; the text comparer cannot open it.",
  },
  hoverAccept: {
    title: "Word / phrase change",
    titleSentence: "Sentence change",
    titleInsert: "Only on compare side",
    titleDelete: "Only in work",
    badgeInsert: "Insert",
    badgeDelete: "Delete",
    badgeReplace: "Replace",
    work: "Work",
    compare: "Compare",
    workEmptyInsert: "(nothing here in work yet)",
    workEmpty: "(no matching text in work)",
    compareEmptyDelete: "(not present on compare side)",
    compareEmpty: "(no matching text on compare)",
    insertLead: "This compare text will be inserted into work:",
    deleteLead: "This work text will be removed (missing on compare side):",
    apply: "Replace in work",
    applyInsert: "Insert into work",
    applyDelete: "Delete from work",
    applySentence: "Apply sentence",
    dismiss: "Dismiss",
    insertHint: "After apply, work gains this compare text. Use toolbar Undo to revert.",
    deleteHint: "After apply, work loses this span. Use toolbar Undo to revert.",
    appliedToast: "Applied word/sentence change · undo available",
  },
  gitPick: {
    title: "Pick compare file from git",
    hint: "Select a commit, then a path in that tree. Content loads via git show (no temp checkout).",
    commits: "Commits",
    filterCommits: "Filter…",
    noCommits: "No commits yet",
    treeAt: "Files at {ref}",
    selected: "Selected: {path}",
    confirm: "Use as compare side",
  },
  gutter: {
    pullLine: "Pull this line (compare → project)",
    pullBlock: "Pull contiguous block",
    pullHunk: "Pull this hunk",
  },
  tree: {
    showDot: "Show dotfiles / folders",
    showDotShort: "dotfiles",
    empty: "Import a project to see the tree",
    compareDir: "Compare this folder (dot dirs skipped by default)",
    collapseAll: "Collapse all",
    expandTop: "Expand top level",
    addToCompare: "Add to compare",
    newCompare: "New compare",
    newCompareFailed: "Could not open comparer",
  },
  store: {
    ready: "Ready",
    uploading: "Uploading…",
    project: "Project {id}",
    projectGit: "Project {id} (git)",
    importingGit: "Importing from git…",
    accepted: "Accepted {granularity} → rev {revision}",
    acceptedAll: "Accepted all → rev {revision}",
    appliedCompare: "Pulled compare → work ({path})",
    appliedCompareAll: "Pulled full compare file → work ({path})",
    actionPath: "{action} {path}",
    undoOk: "Undo ok",
    saved: "Saved {path}",
    compiling: "Compiling ({kind})…\n",
    compileTimeout: "Compile timeout",
    compileTimeoutError: "compile timeout waiting for job",
    compileOk: "Compile OK",
    latexdiffOk: "latexdiff PDF OK",
    compileFailed: "Compile failed",
    compileFailedMsg: "compile failed",
    rootSet: "Root: {path}",
    committed: "Committed {sha}",
    restored: "Discarded uncommitted changes",
    zoneAdded: "Zone added: {name}",
    zoneActivated: "Compare zone activated",
    zoneCleared: "Compare zone cleared",
    zoneDeleted: "Compare zone deleted",
    supplementOk: "Added {n} file(s) to project",
  },
  git: {
    commitMsg: "Message",
    commitPlaceholder: "paper-diff: accept changes",
    commitBtn: "Commit",
    unbound: "Local git not ready (import a project first)",
    discard: "Discard uncommitted",
    discardConfirm: "Discard all uncommitted changes in the project?",
    discarded: "Discarded uncommitted changes",
    log: "History",
    dirtyFiles: "Dirty files",
    clean: "Working tree clean",
    refresh: "Refresh",
    zoneFromCommit: "Zone",
    createZone: "Zone",
    selectA: "A",
    selectB: "B",
    compare: "Compare",
    compareCommits: "Compare commits",
    runCompare: "Run compare",
    compareFiles: "Commit diff",
    needTwoRefs: "Select commit A and B first",
    compareOk: "Diff {base}…{revised}: {n} files",
    zoneFromCommitOk: "Zone from commit: {name}",
    setBase: "Set as left (A)",
    setRevised: "Set as right (B)",
    openPreview: "Preview",
    clearPreview: "Exit commit preview",
    baseLabel: "A",
    revisedLabel: "B",
  },
  lang: {
    zh: "中文",
    en: "English",
    switch: "Language",
  },
  settings: {
    title: "Settings",
    sectionAppearance: "Appearance",
    sectionLanguage: "Language",
    sectionWorkbench: "Workbench",
    sectionAbout: "About",
    theme: "Theme",
    themeSystem: "System",
    themeDark: "Dark",
    themeLight: "Light",
    themeHint:
      "System follows the OS light/dark preference automatically.",
    showFiles: "Show side bar",
    showPdfPane: "Show PDF-related layout by default",
    showBottom: "Show bottom output",
    showToolTips: "Show tooltips",
    compactWorkbench: "Compact workbench (experimental)",
    autoSave: "Auto-save (flush after 3s idle)",
    wordWrap: "Word wrap",
    wordWrapHint: "Wrap editor/comparer lines to viewport (toggle Alt/Option+Z)",
    wordHoverAccept: "Word/phrase hover accept",
    wordHoverAcceptHint:
      "On small files, hover highlighted words/phrases to pull compare → work. Large-file tiers keep word units off.",
    openOutputTab: "Open output tool",
    openPdfTab: "Open PDF tool",
    exportAcceptLog: "Export accept log",
    aboutBody:
      "paper-diff: project / compare-zone workbench. Language and theme prefs are stored in this browser.",
  },
} as const;
